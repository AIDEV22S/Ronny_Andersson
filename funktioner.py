...
medlemsök = []
Database1.session = Database1.Session()


# Här skapas anslutningen till databasen och gör vårat table tillgängligt för t.ex inserts, deletes och selects
...
class Funktioner:
    ...
    def delete(cls, id):
        ...
        results = Database1.session.query(Medlem).filter(
            Medlem.id == id).all()
        # Skapar en query med en sql-sökning i som man kan loopa igenom
        # Letar om någon i registret har det specificerade id:t användaren har satt som input. Om det hittas så
        # raderar den medlemmen.
        for medlem1 in results:
        ...
        if not results:  # Om inte medlems-id hittas i results
            ...
            no_match_id = 1
            return no_match_id
        # Detta kommer att göra att en if-sats i radera_medlem() funktionen blir sann och som gör ett nytt fönster där
        # den ger ett error-meddelande som säger att medlemMen inte hittades

    @classmethod
    def kontroll_id(cls, id):

        results = Database1.session.query(Medlem).filter(Medlem.id == id).all()
        for medlem1 in results:
            if medlem1 in results:
                return id
        ...
        if not results:  # Om inte medlems-id hittas i results
            ...
            no_match_id = 1
            return no_match_id
            # som säger att medlemmen inte hittades
    ...
    def uppdatera(cls, id, avgift):
        ...
        update_1 = update(Medlem, values={Medlem.avgift: avgift}).where(Medlem.id == id)
        # Uppdaterar attributet Medlem.avgift till antingen Betald eller Inte betald beroende på vad man valde
    ...
    def add(cls, förnamn, efternamn, gatuadress, postnummer, postadress, avgift):
        ...
        while True:
            ...
            if avgift == 1:
                avgift = 'Inte betald'
                break
        # Kontrollerar om något av fälten förnamn eller efternamn är tomma. Programmet kallar på en klassmetod som ger
        ...
        objekt = 0
        medlemmar = []
        # Kollar om objektnamnet är ledigt. Om det redan finns ett objekt i listan med det aktuella namnet så ökar den
        # siffran med ett varje gång.
        for y in medlemmar:
            if y == objekt:
                objekt += 1
            else:
        ...
        objekt = Medlem(förnamn, efternamn, gatuadress, postnummer, postadress, avgift)
    ...
    def alla_medlemmar(cls):
        ...
        results = session1.query(Medlem).all()
        # Letar fram alla medlemmar i registret och appendar dem till en lista som skrivs ut i GUI med en for loop
    ...
    def sök_förnamn(cls, förnamn):
        ...
        results = session1.query(Medlem).filter(
            Medlem.förnamn == förnamn).all()
        # Alla matchningar med både för och efternamn i medlemsregistret appendas till listan "medlemsök"
        for medlem in results:
            medlemsök.append(medlem.förnamn)
    ...
    def sök_efternamn(cls, efternamn):
        ...
        session1 = Database1.Session()
        results = session1.query(Medlem).filter(Medlem.efternamn == efternamn).all()
    ...
    def sök_gatuadress(cls, gatuadress):
        ...
        session1 = Database1.Session()
        results = session1.query(Medlem).filter(Medlem.gatuadress == gatuadress).all()
    ...
    def sök_id(cls, id):
        ...
        session1 = Database1.Session()
        results = session1.query(Medlem).filter(Medlem.id == id).all()
...
class sub_windows:
    ...
    def radera_medlem(cls):
        # Skapar ett temporärt sub-window
        while True:
            layout3 = [[sg.Text('ID:'), sg.Input(k='-ID-'), sg.Button('Radera')]]
            ...
            if event == 'Radera':
                result = Funktioner.delete(values['-ID-'])
                # Skickar in integer till delete() som tar reda på vilken self.id den tillhör och sparar det i variabeln
                # result som ska avgöra om ett error-fönster ska visas eller inte beroende på om medlems-id:t finns
                # eller inte
                ...
                if result != 1:
                    layout5 = [[sg.Text(f'Medlemmen har tagits bort ur registret')]]
    ...
    def lägg_till(cls):
        while True:
            ...
            layout2 = [[sg.Text('För att kunna lägga till en ny medlem så måste förnamn och efternamn fyllas i.\nResten'
                                ' av attributen är valfria.')],  # Skapar ett temporärt subwindow
    ...
    def error_parametrar(
        ...
        while True:
            ...
            layout5 = [[sg.Text('Förnamn och efternamn måste vara ifyllda')]]
    ...
    def uppdatera_betalning(cls):
        while True:
            ...
            if event == sg.WIN_CLOSED:
                window12.close()
                break
            # Skickar in integer till lägg_int_betalning1() som tar reda på vilken self.id den tillhör
            if event == 'Uppdatera':
                result = Funktioner.kontroll_id(values['-ID-'])
                window12.close()
                # Om användarens input finns i registret så skickas inputen som en parameter till funktionen
                # uppdatera_betalning1
                if result != 1:
    ...
    @classmethod
    def uppdatera_betalning1(cls, id):
        while True:
